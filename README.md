...
- Supports sorting options: ?sort=date (default) or ?sort=alphabetical
- Shows author post counts in sidebar
`ash
curl http://localhost:5000/
curl http://localhost:5000/?sort=alphabetical
`

#### 2. **GET /create - Create Post Form**
- Display form to create new post

#### 3. **POST /create - Create Post**
- Creates new post (double-writes to MongoDB + Cassandra)
- Fields: title, content, author
- Redirect to home after creation

#### 4. **GET /post/<post_id> - Post Details**
- Display single post with comments
- Comments sorted by date (database-level)

#### 5. **GET /edit/<post_id> - Edit Form**
- Display form to edit existing post

#### 6. **POST /edit/<post_id> - Update Post**
- Updates post (double-writes to MongoDB + Cassandra)
- Updates all fields: title, content, author

#### 7. **POST /delete/<post_id> - Delete Post**
- Deletes post and associated comments (from both databases)

#### 8. **POST /post/<post_id>/comment - Add Comment**
- Adds comment to post (double-writes)
- Fields: commenter, comment

#### 9. **GET /api/author-stats - Author Statistics**
- Returns post count per author (aggregated in database)
`ash
curl http://localhost:5000/api/author-stats
# Returns: {\"John\": 5, \"Jane\": 3}
`

#### 10. **GET /api/stats - Database Statistics**
- Returns document counts for MongoDB and Cassandra
`ash
curl http://localhost:5000/api/stats
# Returns: {
#   \"mongodb\": {\"posts\": 10, \"comments\": 25},
#   \"cassandra\": {\"posts\": 10, \"comments\": 25}
# }
`

## Database Queries (Database-Level)

### 1. Author Post Counts
\\\python
# MongoDB Aggregation Pipeline - RUNS IN DATABASE
pipeline = [
    {
        '\': {
            '_id': '\',
...
\\\python
# DATABASE-LEVEL SORTING (not Python)
sort_direction = -1  # descending
posts = posts_collection.find().sort('created_at', sort_direction)
\\\

### 3. Sort Posts Alphabetically
\\\python
# DATABASE-LEVEL SORTING with collation
posts = posts_collection.find().collation({'locale': 'en'}).sort('title', 1)
\\\
